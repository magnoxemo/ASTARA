...
class Condenser(Base):
    ...
    def DW_steam(self):
        ...
        dtdWsteam = (
            self.W_turbine + self.W_otherthanturbine - self.W_condensate - self.Wss
        )
    ...
    def Dsteamenthalpy(self, temp_otherthanturbine: float, pressure: float):
        dtdWSHS = (
            ...
            * PropsSI("H", "T", temp_otherthanturbine, "P", pressure, "water")
    ...
    def DHotWellEnthalpy(self, Enthalpy_oxygen, pressure):
        dtdWwHw = (
            ...
            - self.W_hotwater * PropsSI("H", self.T_steamout, "P", pressure, "water")
        )
        return dtdWwHw
...
while t < 1000:
    ...
    Condenser.W_turbine = Condenser.integrator(
        Condenser.DW_steam, [], intitial_cond=Condenser.W_turbine, time_step=dt
    )
    Condenser.T_hotwater = Condenser.integrator(
        Condenser.DT_hotwater, [], intitial_cond=Condenser.T_hotwater, time_step=dt
    )
    ...
    if t > 50 and t < 800:
        ...
        Condenser.T_coldwater = (
            Condenser.T_coldwater * 0.01 * np.exp((t - 800) / 100000)
        )
    else:
