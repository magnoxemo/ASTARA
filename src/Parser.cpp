...
double astara::Function::parseExpression() {
...
double astara::Function::parseTerm() {
...
double astara::Function::parseFactor() {
...
double astara::Function::parseUnary() {
  skipWhitespace();
  if (match('+'))
    return parseUnary();
  if (match('-'))
...
double astara::Function::parsePrimary() {
  skipWhitespace();
...
std::string astara::Function::parseIdentifier() {
  size_t start = _pos;
  while (_pos < _expression.size() &&
...
double astara::Function::getVariable(const std::string &name) {
...
double astara::Function::callFunction(const std::string &name, double arg) {
  return _functions.at(name)(arg);
}

bool astara::Function::match(char c) {
...
char astara::Function::peek() const {
  return _pos < _expression.size() ? _expression[_pos] : '\0';
}

void astara::Function::skipWhitespace() {
