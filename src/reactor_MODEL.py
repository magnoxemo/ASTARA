...
class Reactor:
    ...
    def __init__(
        ...
        Fission_power_fraction: float,
        Average_heat_transfer_factor: float,
        Coolant_mas_flow_rate: float,
        Coolant_mass_in_twofluid_nodes: float,
        Clod_leg_water_mass: float,
        fuel_nodal_mass: float,
        Hot_leg_water_mass: float,
        Lower_plenum_water_mass: float,
        Coolant_node_mass: float,
        upper_plenum_water_mass: float,
        Reactor_core_power: float,
        Cold_leg_temp: float,
        hot_leg_tem: float,
        Fluid_temp_in_lower_plenum: float,
        Fluid_temp_in_upper_plenum: float,
        Fuel_temp_in_each_node: list,
        Moderatro_temp_in_nodes: list,
        Steam_gen_inlet_temp: float,
        Reactivity_coefficient_coolant: float,
        Reactivity_coefficient_fuel: float,
        beta: list,
        neutron_group_const: list,
        neutron_generation_time: float,
        Total_reactivity: float,
        external_reactivity: float,
        TempF0_initial_cond: list,
        TempM0_initial_cond: list,
    ) -> None:
    ...
    def constitutive_cal(self):
        sum_mod1 = (
            self.T_moderator_node1 + self.T_moderator_node2 + self.T_moderator_node3
        )
        ...
        self.Ro = (
            self.Ro_ex
            + self.alpha_c * (sum_mod1 - sum_mod0) / 6
            + self.alpha_f * (sum_f1 - sum_f0) / 3
        )
    ...
    def dTf1(self):
        ...
        Tf1 = self.F_r * self.P0 * (self.P / self.P0) / (
            self.M * self.C_pf
        ) + self.h * self.A * (self.T_moderator_node1 - self.T_fuel_node1) / (
            self.M * self.C_pf
        )
    ...
    def dTf2(self):
        ...
        Tf2 = self.F_r * self.P0 * (self.P / self.P0) / (
            self.M * self.C_pf
        ) + self.h * self.A * (self.T_moderator_node3 - self.T_fuel_node2) / (
            self.M * self.C_pf
        )
    ...
    def dTf3(self):
        ...
        Tf3 = self.F_r * self.P0 * (self.P / self.P0) / (
            self.M * self.C_pf
        ) + self.h * self.A * (self.T_moderator_node5 - self.T_fuel_node3) / (
            self.M * self.C_pf
        )
    ...
    def dTmol1(self):
        ...
        TMol1 = (
            (1 - self.F_r) * self.P0 * (self.P / self.P0) / (self.M * self.C_pf)
            + self.h
            * self.A
            * (-self.T_moderator_node1 + self.T_fuel_node1)
            / (self.M * self.C_pf)
    ...
    def dTmol3(self):
        ...
        TMol3 = (
            ...
            + self.h
            * self.A
            * (-self.T_moderator_node3 + self.T_fuel_node2)
            / (self.M * self.C_pf)
    ...
    def dTmol5(self):
        ...
        TMol5 = (
            ...
            + self.h
            * self.A
            * (-self.T_moderator_node5 + self.T_fuel_node3)
            / (self.M * self.C_pf)
