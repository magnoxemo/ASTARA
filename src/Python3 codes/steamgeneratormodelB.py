...
class PrimaryFluidLump():
    ...
    def __init__(self,Pressure:float,PrimaryLump1Temperature:float,
        ...
        self.Spm1=np.pi*self.length*self.outer_diameter*self.number_of_utube*\
        (self.frist_lump_length/self.length)
        
        self.Spm2=np.pi*self.length*self.outer_diameter*self.number_of_utube*\
...
class MetalLump():
    def __init__(self,MetalLump1Temperature:float,MetalLump2Temperature:float):
        ...
        self.Ums=11186.216           #effective heat transfer co-efficient between water and steel     
        ...
        self.Mass1=self.metaldensity*self.number_of_utube*self.length*np.pi*\
        (self.outer_diameter**2-self.inner_diameter**2)*\
        (self.frist_lump_length/self.length)/4
        
        self.Mass2=self.metaldensity*self.number_of_utube*self.length*np.pi*\
        (self.outer_diameter**2-self.inner_diameter**2)*\
        (1-self.frist_lump_length/self.length)/4
        
        self.Sms1=np.pi*self.length*self.outer_diameter*self.number_of_utube*\
        (self.frist_lump_length/self.length)
        
        self.Sms2=np.pi*self.length*self.outer_diameter*self.number_of_utube*\
        (1-self.frist_lump_length/self.length)

    def DTm1(self,PrimaryFluidLump:object,SecondaryFluidLump:object):

        val1=PrimaryFluidLump.Upm*PrimaryFluidLump.Spm1*(PrimaryFluidLump.Tp1-self.Tm1)
    ...
    def DTm2(self,SecondaryFluidLump:object):
        ...
        val1=PrimaryFluidLump.Upm*PrimaryFluidLump.Spm2*(PrimaryFluidLump.Tp2-self.Tm2)
...
class SecondaryFluidLump():
    ...
    def __init__(self,inletTemperature:float,Pressure:float,FeedWaterFlowRate:float,
                EvaporationRate:float,SteamExitRate:float):
        
        self.Tfi=inletTemperature
        self.P=Pressure
        self.Tsat=PropsSI('T',"P",self.P,"Q",1,'water')
        self.hfg=PropsSI('H',"P",self.P,"Q",1,'water')-PropsSI('H',"P",self.P,"Q",0,'water')
        self.Vfg=PropsSI('V',"P",self.P,"Q",0,'water')-PropsSI('V',"P",self.P,"Q",1,'water')
    ...
    def DPressure(self,MetalLump:object,PrimaryFluidLump:object):
        ...
        val1=MetalLump.Ums*MetalLump.Sms1 *(MetalLump.Tm1+MetalLump.Tm2-self.Tsat*2)
        val2=self.Wfi*PrimaryFluidLump.Cp2*self.Tfi
        val3=PropsSI('H',"P",self.P,"Q",1,'water')*(self.Wsg-self.Wso)
        val4=PropsSI('H',"P",self.P,"Q",0,'water')*(self.Wfi-self.Wsg)+self.Wso*PropsSI('H',"P",self.P,"Q",1,'water')

        dtdP=(val1+val2-val3-val4)/(self.Msw*self.dHfdTstat(self.P)+self.Mss*self.dHgdTstat(self.P))
