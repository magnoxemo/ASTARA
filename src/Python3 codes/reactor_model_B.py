...
class CoreInside():
    def __init__(self,TemperatureFuel:list,TemperatureModerator:list,Power:float,Precursor:float,Pressure:float):
    ...
    def Reacivity(self,FuelTempSum:float,ModeratorTempSum):
        ...
        self.reactivity=self.ExternalReactivity+self.Alpha_f*(self.tempfuel-FuelTempSum)/3+self.Alpha_m*(self.tempmod-ModeratorTempSum)/6
    ...
    def DTm01(self,LowerPlenum:object):
        ...
        dtdTm01=(1-self.Fr)*self.NominalPower*self.PowerRatio/(self.Mmo*self.Cpc)+\
            self.h*self.area*(self.Tf1-self.Tmo1)/(self.Mmo*self.Cpc)+(LowerPlenum.Tlp-self.Tmo1)/self.time_constmo
        return dtdTm01 
