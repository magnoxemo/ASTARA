class reactor_primary_coolant_pump():
    ...
    def __init__(self,T_po:float,pipe_diameter:float,piping_length:float,friction_factor:float,moment_of_innertia:float,pump_speed:float,flow_rate:float,power_delivered_to_shaft:float,const:list):
    ...
    def DN(self,T_po:float):
        self.water_density=PropsSI('D',"T",T_po,'Q',0,'water')
        DNp=(self.Pd-self.g*self.water_density*self.Q*self.H)/(self.N*self.I*4*np.pi**2)
