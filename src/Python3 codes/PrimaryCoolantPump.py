...
class reactor_primary_coolant_pump():
    ...
    def __init__(self,area_of_piping:float,piping_length:float,friction_factor:float,
                 ...
                 developed_pump_head:float,const:list):
    ...
    def DNpdt(self):
        ...
        self.Th=self.water_density*self.g*self.Qp*self.Hd/(np.pi*2*self.Np)
        DNp=self.Td-self.Th
        return DNp
...
reactor_primary_coolant_pump=reactor_primary_coolant_pump(area_of_piping=10,piping_length=13,
                                                          friction_factor=1,moment_of_innertia=800,actual_flow_rate=850,norminal_flow_rate=8498,
                                                          norminal_pump_speed=1015450,actual_pump_speed=842,power_delivered_to_shaft=6,developed_pump_head=282,
...
while t<100:
    ...
    Q=reactor_primary_coolant_pump.integrator(reactor_primary_coolant_pump.DQdt,[],intitial_cond=reactor_primary_coolant_pump.Qp,time_step=dt)
    N=reactor_primary_coolant_pump.integrator(reactor_primary_coolant_pump.DNpdt,[],intitial_cond=reactor_primary_coolant_pump.Np,time_step=dt)
