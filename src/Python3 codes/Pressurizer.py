...
class Pressurizer:
    ...
    def DPp(self, DTemp_list: list, Temp_coldleg):
        ...
        C1p = (
            ...
            / PropsSI("D", "P", self.Pressure, "Q", 1, "water")
        ) - 1
        ...
        c = (
            Vw * rho_w * (k3p + (k4p * self.Pressure) / self.J_p)
    ...
    def Dlw(self, DTemp_list: list, Temp_coldleg):
        ...
        C2p = (
            self.area * (self.l - self.l_w) * (C1p + 1) * k2p
            + self.area * self.l_w * k1p
        )

        a = (self.area * (self.l - self.l_w) * k2p - C1p / C2p) * self.DPp(
            DTemp_list, Temp_coldleg
        )
        b = (m_surge / C1p) + (
            (C2p * self.DPp(DTemp_list, Temp_coldleg)) - m_surge - self.m_spray
        ) / (C1p) ** 2
