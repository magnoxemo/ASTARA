...
class Condenser_Model_A():
    def __init__(self,exhust_flow_rate_lp:float,exhust_temp_lp:float,exhust_temp_pressure:float,Condenser_outlet_temp:float,
                 Temp_cold_water_in:float,Temp_hot_water_out:float):
        ...
        self.deltaT=(self.T_hotwater-self.T_coldwater)/np.log((self.Ti-self.T_coldwater)/(self.Ti-self.T_hotwater))
    ...
    def Dhout(self):
        ...
        dtdho=(self.W1+self.W3)*(self.hf-self.hout)/(self.Lw*area*self.rho_w)
        return dtdho
...
class Condenser_Model_B():
    ...
    def Dhs(self,temp_otherthanturbine:float,pressure_otherthanturbine:float):
        ...
        self.Hs=PropsSI('H','T',self.T_steamin,'P',self.Ps,'water')
        dtdWSHS=(self.W_turbine*self.Hs+self.W_otherthanturbine*PropsSI("H",'T',temp_otherthanturbine,'P',pressure_otherthanturbine,'water')-(self.Wc+self.Wss)*self.Hs)/self.WaterLevel
    ...
    def Dhw(self,Enthalpy_oxygen,pressure):
        dtdWwHw=self.W_coldwater*self.Hcw+self.W_bubblingoxygen*Enthalpy_oxygen-self.Ww*PropsSI('H',self.T_steamout,'P',pressure,'water')
        return dtdWwHw
    def DTtube(self):
...
Condenser=Condenser_Model_B(steampressure=100e4,airpressure=89e3,hot_well_diametr=20)
...
while t<100:
    ...
    Condenser.W_turbine=Condenser.integrator(Condenser.DWs,[],intitial_cond=Condenser.W_turbine,time_step=dt)
    Condenser.T_hotwater=Condenser.integrator(Condenser.DTtube,[],intitial_cond=Condenser.T_hotwater,time_step=dt)
    ...
    if t>50 and t<800:
        ...
        Condenser.T_coldwater=Condenser.T_coldwater*0.001*np.exp((t-800)/100000)
    else:
