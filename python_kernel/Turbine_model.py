...
class Nozzle_chest:
    ...
    def __init__(
        ...
        steam_density_chest: float,
        nozzle_chest_enthalpy: float,
        Kc_hp: float,
    ...
    def _Whp(self):
        self.Whp1 = self.Kc_hp * np.sqrt(
            self.P_c * self.rou_c - Reheater.P * HighPressureTurbine.rou_exit
        )

    def _dh_c(self):
        Dhc = (
            ((self.W_main * self.h_sd - self.Whp1 * self.h_c) / (self.rou_c * self.V_c))
            + (self.P_c / self.rou_c**2) * self._drou_c()
        ) / (1 - self.k1)
...
class Moisture_seperator:
    ...
    def _wmss(self):
        ...
        self.W_mss = (
            (self.W_hp1 - self.Wbhp) * (self.h_hpex - self.h_f) / self.h_fg
...
class Reheater:
    ...
    class Reheater_steam:
        ...
        def _dQr(self):
            ...
            dQr = (
                (self.T_steam - self.T_r) * (self.w_2nd + self.W_ro) * self.H
                - 2 * self.Q
            ) / (2 * self.tau_2)
...
class HighPressureTurbine:
    def __init__(
        self,
        exit_steam_density: float,
        inlet_flow_rate: float,
        exit_flow_rate_to_MS: float,
        exit_flow_rate_to_heater: float,
        time_const: float,
...
class LowPressureTurbine:
    def __init__(
        ...
        self.Tau = time_const
        self.Wlp_in = (
            inlet_flow_rate  # this comes from the moisture seperator and reheater
        )
